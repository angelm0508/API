...
{
    public class FacturaCompraDetalleControllerTests
    {
        private readonly Mock<IFacturaCompraDetalleApplication> _applicationMock;
        private readonly FacturaCompraDetalleController _controller;

        public FacturaCompraDetalleControllerTests()
        {
            _applicationMock = new Mock<IFacturaCompraDetalleApplication>();
            _controller = new FacturaCompraDetalleController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<FacturaCompraDetalleDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<FacturaCompraDetalleDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1, 1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1, 1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<FacturaCompraDetalleDTO>>(notFound.Value);
        ...
        {
            var dto = new FacturaCompraDetalleDTO { Entry = 1, NoLinea = 1, CodArticulo = "ART1" };
            var respuesta = new Respuesta<FacturaCompraDetalleDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1, 1)).ReturnsAsync(respuesta);
        ...
        [Fact]
        public async Task ObtenerPorFacturaCompra_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<IEnumerable<FacturaCompraDetalleDTO>> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ObtenerPorFacturaCompraAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.ObtenerPorFacturaCompra(1);
        ...
        [Fact]
        public async Task ObtenerPorFacturaCompra_DevuelveOk_CuandoResultadoEsExitoso()
        {
            var respuesta = new Respuesta<IEnumerable<FacturaCompraDetalleDTO>>
            {
                Resultado = true,
                Dato = new List<FacturaCompraDetalleDTO> { new FacturaCompraDetalleDTO { Entry = 1, NoLinea = 1 } }
            };
            _applicationMock.Setup(a => a.ObtenerPorFacturaCompraAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new FacturaCompraDetalleCrearDTO { Entry = 1, CodArticulo = "ART1" };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, 1, new FacturaCompraDetalleActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1, 1))
                .ReturnsAsync(new Respuesta<FacturaCompraDetalleDTO> { Resultado = true, Dato = new FacturaCompraDetalleDTO { Entry = 1, NoLinea = 1 } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, 1, It.IsAny<FacturaCompraDetalleActualizarDTO>())).ReturnsAsync(respuestaUpdate);
