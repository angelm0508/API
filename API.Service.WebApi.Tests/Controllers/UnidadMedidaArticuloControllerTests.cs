...
{
    public class UnidadMedidaArticuloControllerTests
    {
        private readonly Mock<IUnidadMedidaArticuloApplication> _applicationMock;
        private readonly UnidadMedidaArticuloController _controller;

        public UnidadMedidaArticuloControllerTests()
        {
            _applicationMock = new Mock<IUnidadMedidaArticuloApplication>();
            _controller = new UnidadMedidaArticuloController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<UnidadMedidaArticuloDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<UnidadMedidaArticuloDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<UnidadMedidaArticuloDTO>>(notFound.Value);
        ...
        {
            var dto = new UnidadMedidaArticuloDTO { Entry = 1, Codigo = "ME1" };
            var respuesta = new Respuesta<UnidadMedidaArticuloDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<UnidadMedidaArticuloDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var datos = new List<UnidadMedidaArticuloDTO> { new UnidadMedidaArticuloDTO { Entry = 1, Codigo = "ME1" } };
            var respuesta = new Respuesta<IEnumerable<UnidadMedidaArticuloDTO>> { Resultado = true, Dato = datos };
            _applicationMock.Setup(a => a.ObtenerContengaNombreAsync("Med")).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new UnidadMedidaArticuloCrearDTO { Codigo = "ME1" };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, new UnidadMedidaArticuloActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1))
                .ReturnsAsync(new Respuesta<UnidadMedidaArticuloDTO> { Resultado = true, Dato = new UnidadMedidaArticuloDTO { Entry = 1, Codigo = "ME1" } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<UnidadMedidaArticuloActualizarDTO>())).ReturnsAsync(respuestaUpdate);
