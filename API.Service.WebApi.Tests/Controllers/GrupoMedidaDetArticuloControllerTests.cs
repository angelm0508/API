...
{
    public class GrupoMedidaDetArticuloControllerTests
    {
        private readonly Mock<IGrupoMedidaDetArticuloApplication> _applicationMock;
        private readonly GrupoMedidaDetArticuloController _controller;

        public GrupoMedidaDetArticuloControllerTests()
        {
            _applicationMock = new Mock<IGrupoMedidaDetArticuloApplication>();
            _controller = new GrupoMedidaDetArticuloController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<GrupoMedidaDetArticuloDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<GrupoMedidaDetArticuloDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<GrupoMedidaDetArticuloDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El código del detalle de grupo de medida no se encontró.", valor.Mensaje);
        }

        [Fact]
        public async Task Obtener_DevuelveOk_CuandoExiste()
        {
            var dto = new GrupoMedidaDetArticuloDTO { Codigo = 1, CodigoGrpMedida = 1 };
            var respuesta = new Respuesta<GrupoMedidaDetArticuloDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoMedidaDetArticuloDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoMedidaDetArticuloDTO>> { Resultado = true, Dato = new List<GrupoMedidaDetArticuloDTO> { new GrupoMedidaDetArticuloDTO { Codigo = 1 } } };
            _applicationMock.Setup(a => a.ObtenerTodoAsync()).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new GrupoMedidaDetArticuloCrearDTO { CodigoGrpMedida = 1 };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, new GrupoMedidaDetArticuloActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1))
                .ReturnsAsync(new Respuesta<GrupoMedidaDetArticuloDTO> { Resultado = true, Dato = new GrupoMedidaDetArticuloDTO { Codigo = 1 } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<GrupoMedidaDetArticuloActualizarDTO>())).ReturnsAsync(respuestaUpdate);
