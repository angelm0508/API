...
{
    public class GrupoUnidadMedidaArticuloControllerTests
    {
        private readonly Mock<IGrupoUnidadMedidaArticuloApplication> _applicationMock;
        private readonly GrupoUnidadMedidaArticuloController _controller;

        public GrupoUnidadMedidaArticuloControllerTests()
        {
            _applicationMock = new Mock<IGrupoUnidadMedidaArticuloApplication>();
            _controller = new GrupoUnidadMedidaArticuloController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<GrupoUnidadMedidaArticuloDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<GrupoUnidadMedidaArticuloDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<GrupoUnidadMedidaArticuloDTO>>(notFound.Value);
        ...
        {
            var dto = new GrupoUnidadMedidaArticuloDTO { Entry = 1, Nombre = "Grupo Medida 1", BaseMedida = 1 };
            var respuesta = new Respuesta<GrupoUnidadMedidaArticuloDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoUnidadMedidaArticuloDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoUnidadMedidaArticuloDTO>> { Resultado = true, Dato = new List<GrupoUnidadMedidaArticuloDTO> { new GrupoUnidadMedidaArticuloDTO { Entry = 1, BaseMedida = 1 } } };
            _applicationMock.Setup(a => a.ObtenerContengaNombreAsync("Gru")).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new GrupoUnidadMedidaArticuloCrearDTO { Nombre = "Grupo Medida 1", BaseMedida = 1 };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, new GrupoUnidadMedidaArticuloActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1))
                .ReturnsAsync(new Respuesta<GrupoUnidadMedidaArticuloDTO> { Resultado = true, Dato = new GrupoUnidadMedidaArticuloDTO { Entry = 1, BaseMedida = 1 } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<GrupoUnidadMedidaArticuloActualizarDTO>())).ReturnsAsync(respuestaUpdate);
