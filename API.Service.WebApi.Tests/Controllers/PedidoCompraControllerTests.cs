...
namespace API.Service.WebApi.Tests.Controllers
{
    ...
    {
        private readonly Mock<IPedidoCompraApplication> _applicationMock;
        private readonly PedidoCompraController _controller;

        public PedidoCompraControllerTests()
        {
            _applicationMock = new Mock<IPedidoCompraApplication>();
            _controller = new PedidoCompraController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<PedidoCompraDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<PedidoCompraDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<PedidoCompraDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El código del pedido de compra no se encontró.", valor.Mensaje);
        }

        [Fact]
        public async Task Obtener_DevuelveOk_CuandoExiste()
        {
            var dto = new PedidoCompraDTO { Entry = 1, NumDoc = 100, Serie = 1 };
            var respuesta = new Respuesta<PedidoCompraDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<PedidoCompraDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<PedidoCompraDTO>> { Resultado = true, Dato = new List<PedidoCompraDTO> { new PedidoCompraDTO { Entry = 1 } } };
            _applicationMock.Setup(a => a.ObtenerTodoAsync()).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new PedidoCompraCrearDTO { NumDoc = 100, Serie = 1 };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, new PedidoCompraActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1))
                .ReturnsAsync(new Respuesta<PedidoCompraDTO> { Resultado = true, Dato = new PedidoCompraDTO { Entry = 1 } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<PedidoCompraActualizarDTO>())).ReturnsAsync(respuestaUpdate);
