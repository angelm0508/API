...
namespace API.Service.WebApi.Tests.Controllers
{
    public class CotizacionControllerTests
    {
        ...
        {
            var respuesta = new Respuesta<CotizacionDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<CotizacionDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<CotizacionDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El código de la cotización no se encontró.", valor.Mensaje);
        }
        ...
        public async Task Obtener_DevuelveOk_CuandoExiste()
        {
            ...
            var respuesta = new Respuesta<CotizacionDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<CotizacionDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<CotizacionDTO>> { Resultado = true, Dato = new List<CotizacionDTO> { new CotizacionDTO { Entry = 1 } } };
            _applicationMock.Setup(a => a.ObtenerTodoAsync()).ReturnsAsync(respuesta);
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            ...
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<CotizacionActualizarDTO>())).ReturnsAsync(respuestaUpdate);
