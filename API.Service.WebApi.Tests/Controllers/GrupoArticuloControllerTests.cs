...
namespace API.Service.WebApi.Tests.Controllers
{
    public class GrupoArticuloControllerTests
    {
        ...
        {
            var respuesta = new Respuesta<GrupoArticuloDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<GrupoArticuloDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<GrupoArticuloDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El código del grupo de articulo no se encontró.", valor.Mensaje);
        }
        ...
        public async Task Obtener_DevuelveOk_CuandoExiste()
        {
            ...
            var respuesta = new Respuesta<GrupoArticuloDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<GrupoArticuloDTO> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ObtenerAsync("Grupo 1")).ReturnsAsync(respuesta);
        ...
        public async Task ObtenerPorNombre_DevuelveNotFound_CuandoDatoEsNulo()
        {
            ...
            var valor = Assert.IsType<Respuesta<GrupoArticuloDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El nombre del grupo de articulo no se encontró.", valor.Mensaje);
        }
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoArticuloDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<GrupoArticuloDTO>> { Resultado = true, Dato = new List<GrupoArticuloDTO> { new GrupoArticuloDTO { Codigo = 1 } } };
            _applicationMock.Setup(a => a.ObtenerContengaNombreAsync("Gru")).ReturnsAsync(respuesta);
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            ...
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<GrupoArticuloActualizarDTO>())).ReturnsAsync(respuestaUpdate);
