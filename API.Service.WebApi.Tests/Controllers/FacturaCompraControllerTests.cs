...
namespace API.Service.WebApi.Tests.Controllers
{
    ...
    {
        private readonly Mock<IFacturaCompraApplication> _applicationMock;
        private readonly FacturaCompraController _controller;

        public FacturaCompraControllerTests()
        {
            _applicationMock = new Mock<IFacturaCompraApplication>();
            _controller = new FacturaCompraController(_applicationMock.Object);
        }

        [Fact]
        public async Task Obtener_DevuelveBadRequest_CuandoResultadoEsFalso()
        {
            var respuesta = new Respuesta<FacturaCompraDTO> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<FacturaCompraDTO> { Resultado = true, Dato = null! };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);

            var resultado = await _controller.Obtener(1);

            var notFound = Assert.IsType<NotFoundObjectResult>(resultado.Result);
            var valor = Assert.IsType<Respuesta<FacturaCompraDTO>>(notFound.Value);
            Assert.False(valor.Resultado);
            Assert.Equal("El código de la factura de compra no se encontró.", valor.Mensaje);
        }

        [Fact]
        public async Task Obtener_DevuelveOk_CuandoExiste()
        {
            var dto = new FacturaCompraDTO { Entry = 1, NumDoc = 100, Serie = 1 };
            var respuesta = new Respuesta<FacturaCompraDTO> { Resultado = true, Dato = dto };
            _applicationMock.Setup(a => a.ObtenerAsync(1)).ReturnsAsync(respuesta);
        ...
        {
            var respuesta = new Respuesta<IEnumerable<FacturaCompraDTO>> { Resultado = false, Mensaje = "error" };
        ...
        {
            var respuesta = new Respuesta<IEnumerable<FacturaCompraDTO>> { Resultado = true, Dato = new List<FacturaCompraDTO> { new FacturaCompraDTO { Entry = 1 } } };
            _applicationMock.Setup(a => a.ObtenerTodoAsync()).ReturnsAsync(respuesta);
        ...
        {
            var crearDto = new FacturaCompraCrearDTO { NumDoc = 100, Serie = 1 };
        ...
        public async Task ActualizarAsync_DevuelveNotFound_CuandoNoExiste()
        {
            ...
            var resultado = await _controller.ActualizarAsync(1, new FacturaCompraActualizarDTO());
        ...
        public async Task ActualizarAsync_DevuelveBadRequest_CuandoActualizarFalla()
        {
            _applicationMock.Setup(a => a.ObtenerAsync(1))
                .ReturnsAsync(new Respuesta<FacturaCompraDTO> { Resultado = true, Dato = new FacturaCompraDTO { Entry = 1 } });
            var respuestaUpdate = new Respuesta<bool> { Resultado = false, Mensaje = "error" };
            _applicationMock.Setup(a => a.ActualizarAsync(1, It.IsAny<FacturaCompraActualizarDTO>())).ReturnsAsync(respuestaUpdate);
