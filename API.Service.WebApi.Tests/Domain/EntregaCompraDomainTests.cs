...
namespace API.Service.WebApi.Tests.Domain
{
    ...
    {
        private readonly Mock<IRepositorioGenerico<EntregaCompra, int>> _repoEntregaCompraMock;
        private readonly Mock<IRepositorioGenerico<EntregaCompraDetalle, (int Entry, int NoLinea)>> _repoDetalleMock;
        ...
        {
            _repoEntregaCompraMock = new Mock<IRepositorioGenerico<EntregaCompra, int>>();
            _repoDetalleMock = new Mock<IRepositorioGenerico<EntregaCompraDetalle, (int Entry, int NoLinea)>>();
            _repoNumeracionMock = new Mock<IRepositorioGenerico<NumeracionDocumentoDet, int>>();
            _domain = new EntregaCompraDomain(_repoEntregaCompraMock.Object, _repoDetalleMock.Object, _repoNumeracionMock.Object);
        }
        ...
        public async Task InsertarAsync_SerieAutogenerada_AsignaSigNumeroYLoIncrementa()
        {
            ...
            _repoEntregaCompraMock.Setup(r => r.InsertarAsync(It.IsAny<EntregaCompra>()))
                .ReturnsAsync((EntregaCompra c) => { c.Entry = 99; return c; });

            var obj = new EntregaCompra { Serie = 4, NumDoc = 0, TipoObjeto = "algo-que-el-cliente-mando" };
        ...
        public async Task InsertarAsync_SerieManual_RespetaNumDocDelCliente()
        {
            ...
            _repoNumeracionMock.Setup(r => r.ObtenerAsync(4)).ReturnsAsync(serie);
            _repoEntregaCompraMock.Setup(r => r.InsertarAsync(It.IsAny<EntregaCompra>()))
                .ReturnsAsync((EntregaCompra c) => { c.Entry = 1; return c; });

            var obj = new EntregaCompra { Serie = 4, NumDoc = 12345 };
        ...
        public async Task InsertarAsync_SerieManualSinNumDoc_Lanza()
        {
            ...
            var obj = new EntregaCompra { Serie = 4, NumDoc = 0 };

            await Assert.ThrowsAsync<Exception>(() => _domain.InsertarAsync(obj));
            _repoEntregaCompraMock.Verify(r => r.InsertarAsync(It.IsAny<EntregaCompra>()), Times.Never);
        }
        ...
        [Fact]
        public async Task ActualizarAsync_FuerzaTipoObjetoADoce()
        {
            _repoEntregaCompraMock.Setup(r => r.ActualizarAsync(1, It.IsAny<EntregaCompra>())).ReturnsAsync(true);

            var obj = new EntregaCompra { TipoObjeto = "otro-valor" };
