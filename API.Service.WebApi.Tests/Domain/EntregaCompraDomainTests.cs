...
namespace API.Service.WebApi.Tests.Domain
{
    public class EntregaCompraDomainTests
    {
        ...
        {
            var existente = new EntregaCompra { Entry = 7, Cancelado = "N", EstadoInv = "A", Comentario = "nota original" };
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(existente);

            var ok = await _domain.ActualizarAsync(7, new EntregaCompra { Cancelado = "S" });
