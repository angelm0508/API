...
namespace API.Service.WebApi.Tests.Domain
{
    public class FacturaCompraDomainTests
    {
        ...
        private readonly Mock<IRepositorioGenerico<FacturaCompraDetalle, (int Entry, int NoLinea)>> _repoDetalle = new();
        ...
        {
            _domain = new FacturaCompraDomain(_repoHeader.Object, _repoDetalle.Object, _repoNumeracion.Object, _tx.Object, _asiento.Object);
            ...
            _repoHeader.Setup(r => r.InsertarAsync(It.IsAny<FacturaCompra>()))
                .ReturnsAsync((FacturaCompra c) => { c.Entry = 99; return c; });
            _repoDetalle.Setup(r => r.AgregarSinGuardarAsync(It.IsAny<FacturaCompraDetalle>())).Returns(Task.CompletedTask);
        ...
        {
            CodigoObj = "13", Serie = 4, NombreSerie = "Primario",
            SigNumero = sig, FinNumero = fin, Bloqueado = bloqueado, Manual = manual,
            SubTipoDoc = "--", TipoSerie = "N"
        };

        private static FacturaCompraDetalle Linea(string art, string alm, decimal? cant, decimal? precio) =>
        ...
        public async Task InsertarAsync_ConLineas_NumeraAsientaYMarcaEstadoInv()
        {
            ...
            var obj = new FacturaCompra { Serie = 4, FechaDoc = new DateTime(2026, 8, 30) };
            ...
            _repoDetalle.Verify(r => r.AgregarSinGuardarAsync(It.Is<FacturaCompraDetalle>(l => l.Entry == 99 && l.NoLinea == 1)), Times.Once);
            _repoDetalle.Verify(r => r.AgregarSinGuardarAsync(It.Is<FacturaCompraDetalle>(l => l.Entry == 99 && l.NoLinea == 2)), Times.Once);
            // 2 MovimientoRequest correctos
            Assert.Equal(2, _movimientosAsentados.Count);
            Assert.Equal(("13", 99, 1, "ART1", "01", 10m, 25m), (
        ...
        public async Task InsertarAsync_SerieBloqueada_Lanza_YNoInserta()
        {
            ...
            await Assert.ThrowsAsync<Exception>(() => _domain.InsertarAsync(new FacturaCompra { Serie = 4 }, new List<FacturaCompraDetalle>()));
            _repoHeader.Verify(r => r.InsertarAsync(It.IsAny<FacturaCompra>()), Times.Never);
        }
        ...
        public async Task InsertarAsync_SerieManualSinNumDoc_Lanza()
        {
            ...
            await Assert.ThrowsAsync<Exception>(() => _domain.InsertarAsync(new FacturaCompra { Serie = 4, NumDoc = 0 }, new List<FacturaCompraDetalle>()));
        }
        ...
        {
            var existente = new FacturaCompra { Entry = 7, Cancelado = "N", EstadoInv = "A" };
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(existente);

            var ok = await _domain.ActualizarAsync(7, new FacturaCompra { Cancelado = "S", Comentario = "anulado" });

            Assert.True(ok);
            _asiento.Verify(a => a.RevertirAsync("13", 7), Times.Once);
        ...
        public async Task ActualizarAsync_YaCancelado_Lanza()
        {
            ...
            await Assert.ThrowsAsync<Exception>(() => _domain.ActualizarAsync(7, new FacturaCompra { Comentario = "x" }));
        ...
        {
            var existente = new FacturaCompra { Entry = 7, Cancelado = "N", EstadoInv = "A", Comentario = "algo" };
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(existente);

            var ok = await _domain.ActualizarAsync(7, new FacturaCompra { Comentario = null });
        ...
        {
            var existente = new FacturaCompra { Entry = 7, Cancelado = "N", EstadoInv = "A", CodigoSn = "SN-ORIG", MonedaDoc = "GTQ", Comentario = "viejo" };
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(existente);

            await _domain.ActualizarAsync(7, new FacturaCompra { Comentario = "nuevo", CodigoSn = "SN-HACK", MonedaDoc = "USD" });
        ...
        {
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(new FacturaCompra { Entry = 7, EstadoInv = "A", Cancelado = "N" });
        ...
        {
            _repoHeader.Setup(r => r.ObtenerAsync(7)).ReturnsAsync(new FacturaCompra { Entry = 7, EstadoInv = "C", Cancelado = "S" });
            _repoDetalle.Setup(r => r.ObtenerTodoAsync())
                .ReturnsAsync(new List<FacturaCompraDetalle>
                {
