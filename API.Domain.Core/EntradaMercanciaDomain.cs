...
namespace API.Domain.Core
{
    public class EntradaMercanciaDomain : IEntradaMercanciaDomain
    {
        ...
        private readonly IRepositorioGenerico<EntradaMercanciaDetalle, (int Entry, int NoLinea)> _repoDetalle;
        ...
        public EntradaMercanciaDomain(
            IRepositorioGenerico<EntradaMercancia, int> repoEntrada,
        ...
        public async Task<int> InsertarAsync(EntradaMercancia obj, IEnumerable<EntradaMercanciaDetalle> lineas)
        {
            obj.TipoObjeto = TipoObjetoEntradaMercancia;
            obj.EstadoInv = "A";
            obj.Cancelado = "N";
            ...
            if (serie.Bloqueado == "S")
                throw new Exception("La serie está bloqueada y no se puede usar para registrar entradas de mercancía.");
        ...
        public async Task<bool> ActualizarAsync(int id, EntradaMercancia obj)
        {
            var existente = await _repoEntrada.ObtenerAsync(id);
            ...
            if (obj.Cancelado == "S")
            {
                return await _tx.EjecutarAsync(async () =>
                {
                    await _asiento.RevertirAsync(TipoObjetoEntradaMercancia, id);
        ...
        public async Task<bool> EliminarAsync(int id)
        {
            ...
            return await _tx.EjecutarAsync(async () =>
            {
                ...
                return await _repoEntrada.EliminarAsync(id);
            });
        }

        public async Task<EntradaMercancia> ObtenerAsync(int id)
        {
            return await _repoEntrada.ObtenerAsync(id);
        }

        public async Task<IQueryable<EntradaMercancia>> ObtenerTodoAsync()
        {
            return await _repoEntrada.ObtenerTodoAsync();
        }
